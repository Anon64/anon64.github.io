//sources wikipedia, dsame - github
...
const EEE = {
    ...
    //internal codes - never broadcast publicly
...
const ICAO_LIST = {
    //manual addition
    'KWWS': 'SHREVEPORT, LOUISANA',
    'KRNK': 'BLACKSBURG, VIRGINIA',
    //
...
function onload() {
    let sorg = document.getElementById('org');
    for (const [s, l] of Object.entries(ORG)) {
        let option = document.createElement('option');
        option.value = s;
        option.text = `${s} - ${l}`;
        sorg.appendChild(option);
    }

    let seee = document.getElementById('eee');
    for (const [s, l] of Object.entries(EEE)) {
        let option = document.createElement('option');
        option.value = s;
        option.text = `${s} - ${l}`;
        seee.appendChild(option);
    }

    let sloc = document.getElementById('loc');
    for (const [s, l] of Object.entries(SAME_LOC)) {
        let option = document.createElement('option');
        option.value = s;
        option.text = `${s} - ${l}`;
        sloc.appendChild(option);
    }
...
function loadStates() {
    let country = document.getElementById('ctr').value || 'US';
    let ssta = document.getElementById('sta');
    ssta.innerHTML = '';
    for (const [s, l] of Object.entries(window[`${country}_AREA`])) {
        let option = document.createElement('option');
        option.value = s;
        option.text = `${s} - ${l}`;
        ssta.appendChild(option);
    }
}
...
function loadAreas() {
    let country = document.getElementById('ctr').value || 'US';
    let ssta = document.getElementById('sta');
    let sare = document.getElementById('are');
    sare.innerHTML = '';
    for (let [s, l] of Object.entries(window[`SSCCC_${country}`])) {
        if (s.slice(0, 2) != ssta.value) continue;
        s = s.slice(2);
        let option = document.createElement('option');
        option.value = s;
        option.text = `${s} - ${l}`;
        sare.appendChild(option);
    }
}
...
function addLoc() {
    ...
    let P = document.getElementById('loc').value || '0';
    let SS = document.getElementById('sta').value || '00';
    let CCC = document.getElementById('are').value || '000';

    let PSSCCC = `${P}${SS}${CCC.slice(-3)}`;
    if (locations.includes(PSSCCC)) return;

    locations.map(v => (v.slice(1, 6) == `${SS}${CCC.slice(-3)}` ? PSSCCC : v));
    if (!locations.some(v => v.slice(1, 6) == `${SS}${CCC.slice(-3)}`)) {
        locations.push(PSSCCC);
    } else {
...
function formatdur(dur) {
    return `${dur.slice(0, 2)}:${dur.slice(2, 4)}`
}
...
function ddown() {
    let sdur = document.getElementById('duration');
    let ndur;
    const id = TTB.indexOf(cdur);
    if (id == 0) {
        ndur = '0000';
    } else {
        if (getH(cdur) >= 6) {
            let h = getH(cdur);
            let n = h - (h > hlimit ? 0 : 1);
            ndur = `${n > 9 ? n : '0' + n}00`;
        } else {
            ndur = TTB[id - 1]
        }
    ...
    sdur.innerText = `Duration: ${formatdur(ndur)}`;
    cdur = ndur;
}
...
function getHeader() {
    let sorg = document.getElementById('org').value;
    let seee = document.getElementById('eee').value;
    let scallsign = document.getElementById('callsign').value || 'NotEASy';
    let date = new Date();
    return `ZCZC-${sorg}-${seee}-${locations.slice(0, 31).join('-') || '000000'}+${cdur}-${('000' + dayNo(date.getUTCFullYear(), date.getUTCMonth(), date.getUTCDate())).slice(-3)}${('00' + date.getUTCHours()).slice(-2)}${('0' + date.getUTCMinutes()).slice(-2)}-${scallsign.slice(0, 8)}-`;
}

function generate() {
    let result = document.getElementById('result');
    result.innerText = `Result: ${getHeader()}`
}